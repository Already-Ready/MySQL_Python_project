...
def signin(db):
    ...
    if list(db.user.find({'_id':user_id,'pw':user_pw})): #and list(db.user.find({'pw':user_pw})):
        ...
        print("\nplease Check your id or pw")
        print('\n1 : re-signin')
        print('2 : signup')
        req = int(input('\nplease input number want to do : '))
        if req == 1:
            signin(db)
        elif req == 2:
            signup(db)
...
def mystatus(db, user):
    ...
    follower_count = len(db.user.find_one({"_id":user})["follower"])
    following_count = len(db.user.find_one({"_id":user})["followings"])

    #------------------------------------------------------------------------------

    print("\nUser_name : ", user_name)
    print("User_status : ", user_status)
    print("Your follower number is : ", follower_count)
    print("Your following number is : ", following_count)
    if user_follower:
...
def userpage(db, user):
    ...
    while True:
        print()
        print("-"*80)
        print(" 1 : go to Post Interface \n"
              " 2 : go to my status \n"
              " 3 : go to follow \n"
              " 4 : go to unfollow \n"
              " 5 : go to newsfeed \n"
              " 6 : go to comment_post \n"
              " 7 : like \n"
              " 8 : go to wall \n"
              " 9 : quit \n"
              )
